...
def load_config(skip_asr: bool = False) -> dict:
    """加载配置，优先级：环境变量 > 用户 config.yaml > 默认 config.yaml

    Args:
        skip_asr: 文字模式下跳过 ASR 配置校验
    """
    ...
    # 校验必填项
    _validate(config, skip_asr=skip_asr)
...
def _validate(config: dict, skip_asr: bool = False):
    """校验必填配置项"""
    errors = []
    ...
    if not skip_asr:
        ...
        if asr_engine == "whisper-api":
            ...
            if not api_key:
                errors.append("asr.whisper_api.api_key 未设置 (请设置 OPENAI_API_KEY 环境变量，或使用 --text 模式跳过 ASR)")
